...
## 10. Addressing Scalability: Cascades, Snapshots, and DTOs
A common critique of monolithic applications is whether they can truly scale. I have proactively addressed potential bottlenecks in the domain model and response layers.

### 10.1 The Cascade vs. Atomicity Trade-off
I use `CascadeType.ALL` for critical relationships like `Order -> OrderItem -> OrderItemVariant`. 
*   **The Rationale**: In food delivery, "Partial Orders" (e.g., an order saved without its items) are a catastrophic data failure. I prioritized **Transaction Atomicity** over raw insert speed. 
*   **The Optimization**: To mitigate the cost of multiple inserts, I enabled **Hibernate Batching** in my configuration. This allows the JVM to send a single batch of inserts to PostgreSQL, reducing network round-trips for complex orders.

### 10.2 Data Snapshots (De-normalization for Read Scale)
To ensure the system remains fast as the order volume grows, I use a "Snapshot" strategy.
*   **The Strategy**: I don't just link an `OrderItem` to a `Menu` item. I copy the `itemName` and `pricePerUnit` directly into the `order_items` table at the moment of checkout.
*   **The Benefit**: When a user views their order history, I don't need to join the `menu` table. This prevents "historical data drift" and significantly reduces the join complexity of read-heavy queries.

### 10.3 Response Payload Optimization
I strictly avoid returning full JPA entities. Instead, I use a tiered DTO system:
*   **Discovery Phase**: `RestaurantSummaryDTO` contains only the minimal data needed for the list view (Name, Rating, Estimated Time).
*   **Detail Phase**: Full menu data is loaded lazily only when a user clicks a restaurant. 
*   **Fetch Joins**: In my repositories, I use `@EntityGraph` or `JOIN FETCH` for specific high-traffic queries to solve the N+1 problem before it starts.

### 10.4 Infrastructure for Vertical & Horizontal Scale
*   **Redis GEO**: By offloading spatial radius calculations to Redis, I kept the PostgreSQL CPU usage low, allowing the database to focus on transactional integrity.
*   **Stateless Scaling**: Because I use JWTs and avoid server-side sessions, I can spin up multiple instances of this backend behind a simple Nginx or AWS ALB without worrying about session affinity (sticky sessions).
